...
def calc_psnr(sr, hr, scale, rgb_range, benchmark=False):
    ...
    original_psnr = skimage.metrics.peak_signal_noise_ratio(sr.cpu().numpy(), hr.cpu().numpy(), data_range=rgb_range)

    return original_psnr

def calc_psnr_org(sr, hr, scale, rgb_range, benchmark=False):
    if sr.size(-2) > hr.size(-2) or sr.size(-1) > hr.size(-1):
        print("the dimention of sr image is not equal to hr's! ")
        sr = sr[:,:,:hr.size(-2),:hr.size(-1)]
