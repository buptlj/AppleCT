...
class WGAN_VGG(nn.Module):
    ...
    def __init__(self, input_size=64, device="cuda:0"):
    ...
    def g_loss(self, x, y, perceptual=True, return_p=False, mse_w=0.0):
        fake = self.generator(x)
        d_fake = self.discriminator(fake)
        g_loss = -torch.mean(d_fake)
        mse_loss = nn.MSELoss()(fake, y)
        g_loss = g_loss + mse_loss * mse_w
        ...
        if perceptual:
            ...
            p_loss = None
            loss = g_loss
        return (loss, p_loss, mse_loss) if return_p else loss
    ...
    def gp(self, y, fake, lambda_=10):
        ...
        fake_ = torch.FloatTensor(y.shape[0], 1).to(self.device).fill_(1.0).requires_grad_(False)
