...
class MeanShift(nn.Conv2d):
    def __init__(self, rgb_range, rgb_mean, rgb_std, n_colors, sign=-1):
    ...
    def __init__(self, conv, scale, n_feats, bn=False, act=False, bias=True):
        ...
        if (scale & (scale - 1)) == 0:    # Is scale = 2^n?
            for _ in range(int(math.log(scale, 2))):
                ...
                m.append(nn.ConvTranspose2d(n_feats, n_feats, 3, stride=(2, 1), padding=1, output_padding=(1, 0)))
...
class DownBlock(nn.Module):
    def __init__(self, opt, scale, nFeat=None, in_channels=None, out_channels=None):
        ...
        dual_block = [
            nn.Sequential(
                nn.Conv2d(in_channels, nFeat, kernel_size=3, stride=(2, 1), padding=1, bias=False),
        ...
        for _ in range(1, int(np.log2(scale))):
            dual_block.append(
                nn.Sequential(
                    nn.Conv2d(nFeat, nFeat, kernel_size=3, stride=(2, 1), padding=1, bias=False),
                    nn.LeakyReLU(negative_slope=negval, inplace=True)
                )
            )
